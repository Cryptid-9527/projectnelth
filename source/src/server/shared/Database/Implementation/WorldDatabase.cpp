...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_CREATURE_TEXT, "SELECT entry, groupid, id, text, type, language, probability, emote, duration, sound, BroadcastTextID, text_range FROM creature_text", CONNECTION_SYNCH);
    ...
    PrepareStatement(WORLD_SEL_TAXINODE_LEVELS, "SELECT ID, level FROM taxiNode_levels", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_AFFIX_TEMPLATES, "SELECT * FROM affix_template;", CONNECTION_SYNCH);
}
