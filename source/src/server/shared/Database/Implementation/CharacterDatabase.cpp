...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_ENUM, "SELECT c.guid, c.name, c.race, c.class, c.gender, c.playerBytes, c.playerBytes2, c.level, c.zone, c.map, c.position_x, c.position_y, c.position_z, gm.guildid, c.playerFlags, c.at_login, cp.entry, cp.modelid, c.slot, c.equipmentCache, cb.guid, c.slot, c.slot FROM characters AS c LEFT JOIN character_pet AS cp ON c.guid = cp.owner AND cp.slot = c.currentPetSlot LEFT JOIN guild_member AS gm ON c.guid = gm.guid LEFT JOIN character_banned AS cb ON c.guid = cb.guid AND cb.active = 1 WHERE c.account = ? ORDER BY c.slot ASC;", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_SKILLS, "SELECT skill, value, max FROM character_skills WHERE guid = ? ORDER BY skill DESC", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_MATCH_MAKER_RATING, "SELECT matchMakerRating FROM character_pvp_stats WHERE guid = ? AND slot = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_INS_GUILD, "INSERT INTO guild (guildid, name, leaderguid, info, motd, createdate, EmblemStyle, EmblemColor, BorderStyle, BorderColor, BackgroundColor, BankMoney, cashFlowContribution) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_GUILD_CASHFLOW, "UPDATE guild SET cashFlowContribution = ? WHERE guildid = ?", CONNECTION_ASYNC); // 0: uint64, 1: uint32
    ...
    PrepareStatement(CHAR_INS_ARENA_TEAM_MEMBER, "INSERT INTO arena_team_member (arenaTeamId, guid, type) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_ARENA_TEAM, "DELETE FROM arena_team WHERE type = ? AND captainGuid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ARENA_TEAM_MEMBER, "DELETE FROM arena_team_member WHERE type = ? AND guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_ARENA_TEAM_STATS, "UPDATE arena_team SET rating = ?, weekGames = ?, weekWins = ?, seasonGames = ?, seasonWins = ? WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ARENA_TEAM_RANKS, "UPDATE arena_team AT1 LEFT JOIN ((SELECT ROW_NUMBER() OVER (ORDER BY rating DESC, seasonWins DESC) AS rank, arenaTeamId, NAME, TYPE, rating FROM arena_team WHERE TYPE = 2 ORDER BY rating DESC, seasonWins DESC) UNION (SELECT ROW_NUMBER() OVER (ORDER BY rating DESC, seasonWins DESC) AS rank, arenaTeamId, NAME, TYPE, rating FROM arena_team WHERE TYPE = 3 ORDER BY rating DESC, seasonWins DESC) UNION (SELECT ROW_NUMBER() OVER (ORDER BY rating DESC, seasonWins DESC) AS rank, arenaTeamId, NAME, TYPE, rating FROM arena_team WHERE TYPE = 5 ORDER BY rating DESC, seasonWins DESC) ) AT2 ON AT1.arenaTeamId = AT2.arenaTeamId SET at1.rank = AT2.rank;", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ARENA_TEAM_WEEK, "UPDATE arena_team SET weekRating = rating, weekRank = rank WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHARACTER_PVP_STATS, "REPLACE INTO character_pvp_stats (guid, slot, matchMakerRating) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,playerBytes=?,playerBytes2=?,playerFlags=?,"
                     ...
                     "equipmentCache=?,knownTitles=?,actionBars=?,grantableLevels=?,achievementPoint=?,currentPetSlot=?,online=? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_ARENA_TEAM_ID_BY_PLAYER_GUID, "SELECT arena_team_member.arenateamid FROM arena_team_member JOIN arena_team ON arena_team_member.arenateamid = arena_team.arenateamid WHERE guid = ? AND arena_team.type = ? LIMIT 1", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_REP_CHAR_SKILLS, "REPLACE INTO character_skills (guid, skill, value, max) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_STATS, "INSERT INTO character_stats (guid, maxhealth, curr_mana, max_mana, curr_rage, max_rage, curr_focus, max_focus, curr_energy, max_energy, curr_unused, max_unused, curr_runes, max_runes, curr_runicPower, max_runicPower, curr_soulShards, max_soulShards, curr_eclipse, max_eclipse, curr_holyPower, max_holyPower, strength, agility, stamina, intellect, spirit, armor, resHoly, resFire, resNature, resFrost, resShadow, resArcane, blockPct, dodgePct, parryPct, critPct, rangedCritPct, spellCritPct, attackPower, rangedAttackPower, spellPower, resilience, mastery) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    //PvP detection data
    PrepareStatement(CHAR_INS_PVP_DETECTION_DATA, "INSERT INTO character_pvp_detection_data (playerGUID, eventType, DateAndTime, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19, data20) VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ARENA_LOG, "REPLACE INTO `arena_log` (`mapID`, `endTime`, `duration`, `winnerTeam`, `loserTeam`, `winner_dRating`, `winnerRating`, `winnerMMR`, `loser_dRating`, `loserRating`, `loserMMR`, `winpl1`, `winpl2`, `winpl3`, `winpl4`, `winpl5`, `winIP1`, `winIP2`, `winIP3`, `winIP4`, `winIP5`, `win_dmg_1`, `win_dmg_2`, `win_dmg_3`, `win_dmg_4`, `win_dmg_5`, `win_heal_1`, `win_heal_2`, `win_heal_3`, `win_heal_4`, `win_heal_5`, `win_dmgTaken_1`, `win_dmgTaken_2`, `win_dmgTaken_3`, `win_dmgTaken_4`, `win_dmgTaken_5`, `win_healTaken_1`, `win_healTaken_2`, `win_healTaken_3`, `win_healTaken_4`, `win_healTaken_5`, `win_kill_1`, `win_kill_2`, `win_kill_3`, `win_kill_4`, `win_kill_5`, `win_death_1`, `win_death_2`, `win_death_3`, `win_death_4`, `win_death_5`, `win_dRating_1`, `win_dRating_2`, `win_dRating_3`, `win_dRating_4`, `win_dRating_5`, `win_dMMR_1`, `win_dMMR_2`, `win_dMMR_3`, `win_dMMR_4`, `win_dMMR_5`, `losepl1`, `losepl2`, `losepl3`, `losepl4`, `losepl5`, `loseIP1`, `loseIP2`, `loseIP3`, `loseIP4`, `loseIP5`, `lose_dmg_1`, `lose_dmg_2`, `lose_dmg_3`, `lose_dmg_4`, `lose_dmg_5`, `lose_heal_1`, `lose_heal_2`, `lose_heal_3`, `lose_heal_4`, `lose_heal_5`, `lose_dmgTaken_1`, `lose_dmgTaken_2`, `lose_dmgTaken_3`, `lose_dmgTaken_4`, `lose_dmgTaken_5`, `lose_healTaken_1`, `lose_healTaken_2`, `lose_healTaken_3`, `lose_healTaken_4`, `lose_healTaken_5`, `lose_kill_1`, `lose_kill_2`, `lose_kill_3`, `lose_kill_4`, `lose_kill_5`, `lose_death_1`, `lose_death_2`, `lose_death_3`, `lose_death_4`, `lose_death_5`, `lose_dRating_1`, `lose_dRating_2`, `lose_dRating_3`, `lose_dRating_4`, `lose_dRating_5`, `lose_dMMR_1`, `lose_dMMR_2`, `lose_dMMR_3`, `lose_dMMR_4`, `lose_dMMR_5`, `team_yellow`, `team_green`) VALUES (?, NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", CONNECTION_ASYNC);

    // Challenge Mode
    PrepareStatement(CHAR_DEL_CHAR_KEYSTONES, "DELETE FROM character_keystone WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHAR_KEYSTONES, "REPLACE INTO character_keystone (`guid`, `map`, `level`, `affix1`, `affix2`, `affix3`, `affix4`) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHAR_KEYSTONES, "SELECT `map`, `level`, `affix1`, `affix2`, `affix3`, `affix4` FROM character_keystone WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_MYTHIC_HISCORES, "INSERT INTO `character_mythic_hiscores` (`DateAndTime`, `map`, `completionTime`, `level`, `affix1`, `affix2`, `affix3`, `affix4`, `party1_guid`, `party1_spec`, `party1_role`, `party2_guid`, `party2_spec`, `party2_role`, `party3_guid`, `party3_spec`, `party3_role`, `party4_guid`, `party4_spec`, `party4_role`, `party5_guid`, `party5_spec`, `party5_role`) VALUES (NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);

    // Arena system
    PrepareStatement(CHAR_REWARD_ARENA, "INSERT INTO `pending_arena_titles` VALUES (?, ?)", CONNECTION_SYNCH);
    PrepareStatement(CHAR_ARENA_REWARDS, "INSERT INTO `season_gladiators` VALUES (?, ?)", CONNECTION_SYNCH);
    PrepareStatement(CHAR_CLEAN_ARENA_REWARDS, "DELETE FROM `season_gladiators`", CONNECTION_SYNCH);
    PrepareStatement(CHAR_CLEAN_ARENA_TITLES, "DELETE FROM `character_queststatus_rewarded` WHERE `quest` IN (300001, 300002, 300003, 300004)", CONNECTION_SYNCH);
    PrepareStatement(CHAR_ARENA_TITLE_DIST, "SELECT `titleid` FROM `pending_arena_titles` WHERE `guid` = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_ARENA_TITLE_CLEAN, "DELETE FROM `pending_arena_titles` WHERE `guid` = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UNCLAIMED_TITLE_CLEAN, "DELETE FROM `pending_arena_titles` WHERE `titleid` IN (42, 43, 44, 45)", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_INS_HISTORY_MAIL, "INSERT INTO char_history_mail VALUES (?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_HISTORY_MAIL_BY_RECV_GUID, "SELECT Log_GUID, DateTime, send_GUID, recv_guid, money, (IF(item1_entry > 0, 1, 0) + IF(item2_entry > 0, 1, 0) + IF(item3_entry > 0, 1, 0) + IF(item4_entry > 0, 1, 0) + IF(item5_entry > 0, 1, 0) + IF(item6_entry > 0, 1, 0) + IF(item7_entry > 0, 1, 0) + IF(item8_entry > 0, 1, 0) + IF(item9_entry > 0, 1, 0) + IF(item10_entry > 0, 1, 0) + IF(item11_entry > 0, 1, 0) + IF(item12_entry > 0, 1, 0)) AS item_count FROM char_history_mail WHERE recv_guid = ?;", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_HISTORY_MAIL_BY_SEND_GUID, "SELECT Log_GUID, DateTime, send_GUID, recv_guid, money, (IF(item1_entry > 0, 1, 0) + IF(item2_entry > 0, 1, 0) + IF(item3_entry > 0, 1, 0) + IF(item4_entry > 0, 1, 0) + IF(item5_entry > 0, 1, 0) + IF(item6_entry > 0, 1, 0) + IF(item7_entry > 0, 1, 0) + IF(item8_entry > 0, 1, 0) + IF(item9_entry > 0, 1, 0) + IF(item10_entry > 0, 1, 0) + IF(item11_entry > 0, 1, 0) + IF(item12_entry > 0, 1, 0)) AS item_count FROM char_history_mail WHERE send_guid = ?;", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_HISTORY_MAIL_SPECIFIC_LOG, "SELECT * FROM char_history_mail WHERE Log_GUID IN (?)", CONNECTION_SYNCH);

    PrepareStatement(CHAR_INS_HISTORY_TRADE, "INSERT INTO char_history_trade VALUES (?, NOW(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_HISTORY_TRADE_BY_RECV_GUID, "SELECT Log_GUID, DateTime,p1_GUID AS sender,p1_money_offer AS money,(IF(p1_item1_entry > 0, 1, 0)+ IF(p1_item2_entry > 0, 1, 0)+ IF(p1_item3_entry > 0, 1, 0)+ IF(p1_item4_entry > 0, 1, 0)+ IF(p1_item5_entry > 0, 1, 0)+ IF(p1_item6_entry > 0, 1, 0)) AS item_count FROM char_history_trade WHERE(p2_GUID IN(76)) UNION SELECT Log_GUID,DateTime,p2_GUID AS sender,p2_money_offer AS money,(IF(p1_item1_entry > 0, 1, 0)+ IF(p2_item7_entry > 0, 1, 0)+ IF(p2_item8_entry > 0, 1, 0)+ IF(p2_item9_entry > 0, 1, 0)+ IF(p2_item10_entry > 0, 1, 0)+ IF(p2_item11_entry > 0, 1, 0)) AS item_count FROM char_history_trade WHERE p1_GUID IN (?); ", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_HISTORY_TRADE_BY_SEND_GUID, "SELECT  Log_GUID,  DateTime,  p2_GUID AS recipient,  p1_money_offer AS money,  ( IF(p1_item1_entry > 0, 1, 0)  + IF(p1_item2_entry > 0, 1, 0) + IF(p1_item3_entry > 0, 1, 0) + IF(p1_item4_entry > 0, 1, 0) + IF(p1_item5_entry > 0, 1, 0) + IF(p1_item6_entry > 0, 1, 0) ) AS item_count FROM char_history_trade WHERE (p1_GUID IN (76)) UNION  SELECT  Log_GUID,  DateTime, p1_GUID AS recipient,  p2_money_offer AS money,  ( IF(p1_item1_entry > 0, 1, 0)  + IF(p2_item7_entry > 0, 1, 0) + IF(p2_item8_entry > 0, 1, 0) + IF(p2_item9_entry > 0, 1, 0) + IF(p2_item10_entry > 0, 1, 0) + IF(p2_item11_entry > 0, 1, 0) ) AS item_count FROM char_history_trade WHERE p2_GUID IN (?);", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_HISTORY_TRADE_SPECIFIC_LOG, "SELECT * FROM char_history_trade WHERE Log_GUID IN (?) ", CONNECTION_SYNCH);

    PrepareStatement(CHAR_SEL_MACROS_FOR_ACCOUNT, "SELECT * FROM account_data WHERE AccountId IN (?) AND data LIKE '%MACRO%'", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MACROS_FOR_CHARACTER, "SELECT * FROM character_account_data WHERE guid IN (?) AND data LIKE '%MACRO%'", CONNECTION_SYNCH);

    /*
        Player-Interface Reporting systeme
    */
    //Add entry to logs
    PrepareStatement(CHAR_INS_COMPLAINT_LOG, "INSERT INTO `character_complaints` VALUES (NOW(), ?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE DateTime = NOW(), COUNT = COUNT+1, report_details = ?;", CONNECTION_ASYNC);

        //Grab every GUID who has cast a report against a specific GUID
    PrepareStatement(CHAR_SEL_COMPLAINT_LOG_SOURCE_GUIDS_BY_TARGET_GUID, "SELECT DISTINCT source_guid FROM character_complaints WHERE target_guid = (?)", CONNECTION_SYNCH);
        //See outstanding reports from or against a certain guid
    PrepareStatement(CHAR_SEL_COMPLAINT_LOG_BY_TARGET_GUID, "SELECT * FROM character_complaints WHERE target_guid IN (?) ORDER BY DateTime DESC LIMIT ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_COMPLAINT_LOG_BY_SOURCE_GUID, "SELECT * FROM character_complaints WHERE source_guid IN (?) ORDER BY DateTime DESC LIMIT ?", CONNECTION_SYNCH);
        //See the most recent reports
    PrepareStatement(CHAR_SEL_COMPLAINT_LOG_BY_MOST_RECENT, "SELECT * FROM character_complaints ORDER BY DateTime DESC LIMIT ?", CONNECTION_SYNCH);
        //Filtered to a specific type
    PrepareStatement(CHAR_SEL_COMPLAINT_LOG_BY_SPECIFIC_TYPE, "SELECT * FROM character_complaints WHERE type = ? ORDER BY DateTime DESC LIMIT ?", CONNECTION_SYNCH);

        //See who's done the most reporting and been reported the most
    PrepareStatement(CHAR_SEL_COMPLAINT_LOG_TARGETS_BY_MOST_REPORTED, "SELECT target_guid, SUM(count) as sum FROM character_complaints GROUP BY target_guid ORDER BY SUM DESC LIMIT ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_COMPLAINT_LOG_SOURCES_BY_MOST_REPORTED, "SELECT source_guid, SUM(count) as sum FROM character_complaints GROUP BY source_guid ORDER BY SUM DESC LIMIT ?", CONNECTION_SYNCH);
        //Delete logs older than 2 weeks
    PrepareStatement(CHAR_DEL_COMPLAINT_OLDEST, "DELETE from character_complaints WHERE (DateTime < NOW() - INTERVAL 14 DAY)", CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_COMPLAINT_SOURCE, "DELETE from character_complaints WHERE (source_guid = ?)", CONNECTION_SYNCH);

    //Item services
    PrepareStatement(CHAR_SEL_ITEM_INSTANCE_LOOKUP_ALL_OF_PLAYER, "SELECT  `guid`,  `itemEntry`,  `owner_guid`,  `creatorGuid`,  `giftCreatorGuid`,  `count`,  `duration`,  `charges`,  `flags`, LEFT(`enchantments`, 256),  `randomPropertyId`,  `durability`,  `playedTime`, LEFT(`text`, 256) FROM item_instance_buyback iib WHERE iib.itemEntry IN (?) AND iib.owner_guid IN (?) UNION SELECT  `guid`,  `itemEntry`,  `owner_guid`,  `creatorGuid`,  `giftCreatorGuid`,  `count`,  `duration`,  `charges`,  `flags`, LEFT(`enchantments`, 256), `randomPropertyId`,  `durability`,  `playedTime`, LEFT(`text`, 256) FROM item_instance ii WHERE ii.itemEntry IN(? ) AND ii.owner_guid IN(? ); ", CONNECTION_SYNCH);
}
