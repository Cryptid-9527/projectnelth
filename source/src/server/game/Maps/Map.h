...
class Map : public GridRefManager<NGridType>
{
    ...
    public:
        ...
        bool IsAboveWater(float x, float y, float z, LiquidData* data = 0) const;
        bool IsUnderWater(float x, float y, float z) const;

        static uint32 GetAreaIdByAreaFlag(uint16 areaflag, uint32 map_id);
        static uint32 GetZoneIdByAreaFlag(uint16 areaflag, uint32 map_id);
        static void GetZoneAndAreaIdByAreaFlag(uint32& zoneid, uint32& areaid, uint16 areaflag, uint32 map_id);
        static uint32 GetLevelForArea(uint32 area_id, bool max_recommended = false);
        ...
        */
        ...
        std::set<WorldObject*> const& GetWorldObjects() { return i_worldObjects; }


        void ModifyLayerCount(uint32 layer, bool add);
        
        uint32 GetLayerCount(uint8 layer);

        uint8 GetLeastPopulatedLayer(uint8 layer_to_discount);
    ...
    private:
        ...
        std::unordered_map<uint8, uint32> layerCountContainer;
