...
void MapManager::InitNodeLevels()
{
    if (auto stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_TAXINODE_LEVELS))
        if (auto results = WorldDatabase.Query(stmt))
            do
            {
                if (auto fields2 = results->Fetch())
                {
                    uint32 node{ fields2[0].GetUInt32() };
                    uint32 level{ fields2[1].GetUInt32() };
                    m_taxiNodeLevels.insert({ node,level });
                }

            } while (results->NextRow());
}
uint32 MapManager::GetTaxiNodeLevel(uint32 id) const
{
    auto entry = m_taxiNodeLevels.find(id);
    if (entry != m_taxiNodeLevels.end())
        return (entry->second);
