...
enum BattlegroundTimeIntervals
{
    ...
    RESURRECTION_INTERVAL_RATED     = 45000,                // ms
    ...
    RESPAWN_VEHICLE                 = 360,                  // secs
    RESPAWN_IMMEDIATELY             = 0,                    // secs
    SHADOW_SIGHT_SPAWN_TIME         = 90,
...
enum BattlegroundRandomRewards
{
    ...
    BG_REWARD_WINNER_HONOR_STANDARD = 5000,
    BG_REWARD_LOSER_HONOR_STANDARD  = 3500,
};
...
enum BattlegroundCustomHolidayEvents
{
    EVENTS_DISABLED                         = 0,

    //Arenas
    EVENT_ARENAS_ACTIVATE_TORNADO           = 1,
    //ROV SPECIFIC
    EVENT_ARENAS_RV_CROWD_PRESENT           = 2,
    EVENT_ARENAS_RV_CROWD_THROW_TOMATOES    = 4,
};
...
struct BattlegroundScore
{
    BattlegroundScore() : KillingBlows(0), Deaths(0), HonorableKills(0), BonusHonor(0),
        DamageDone(0), HealingDone(0), BgTeam(0), TalentTree(0), HealingTaken(0), DamageTaken(0)
    { }
...
class Battleground
{
    public:
        ...
        uint8 GetArenaSlot() const
        {
            switch (m_ArenaType)
            {
            case ARENA_TYPE_1v1_SOLO:   return NULL;    break;
            case ARENA_TYPE_2v2:        return 0;       break;
            case ARENA_TYPE_3v3:        return 1;       break;
            case ARENA_TYPE_3v3_SOLO:   return NULL;    break;
            case ARENA_TYPE_5v5:        return 2;       break;
        ...
        uint8 ArenaDampeningBitmask(uint8 aType) const;
        ...
        const uint32 getRespawnTime() { return isRatedBattleground() ? RESURRECTION_INTERVAL_RATED : RESURRECTION_INTERVAL; }
        ...
        void AddEmptyPlayerScoresForGUID(uint64 guid, uint32 team);
        ...
        void SetActiveEventMask(uint32 startMaxDist) { m_ActiveEventMask = startMaxDist; }
        uint32 GetActiveEventMask() const { return m_ActiveEventMask; }
        bool HasActiveHolidayEvent(BattlegroundCustomHolidayEvents event) { return (m_ActiveEventMask & uint32(event)); };
        ...
        void DistributeHonorToAll(uint32 winner);
        void RewardEndGameHonorToTeam(bool winner, uint32 TeamID);
        ...
        int32 GetCreatureType(uint64 guid);
    ...
    private:
        ...
        uint8  m_ArenaType;                                 // 2=2v2, 3=3v3, 5=5v5, 6=used for WarGame queue update schedule
