...
void BattlegroundEY::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (ANTICHEAT_time_since_pull > 0) {
            if (ANTICHEAT_time_since_pull < 12000) {
                ANTICHEAT_time_since_pull += diff;
            }
            else {
                ANTICHEAT_time_since_pull = 0;
            }
        }
...
bool BattlegroundEY::SetupBattleground()
{
    ...
    {
        if (!AddCreature(WORLD_TRIGGER, node + 6, TEAM_NEUTRAL, BG_EY_TriggerPositions[node][0], BG_EY_TriggerPositions[node][1], BG_EY_TriggerPositions[node][2] + 1.00f, BG_EY_TriggerPositions[node][3]))
        {
            TC_LOG_ERROR("sql.sql", "Failed to spawn BGCreature capture trigger %u", (node + 6));
            return false;
        }
    }
        // doors
    if (!AddObject(BG_EY_OBJECT_DOOR_A, BG_OBJECT_A_DOOR_EY_ENTRY, 2527.6f, 1596.91f, 1262.13f, -3.12414f, -0.173642f, -0.001515f, 0.98477f, -0.008594f, RESPAWN_IMMEDIATELY)
        ...
        // flags
        || !AddObject(BG_EY_OBJECT_FLAG_NETHERSTORM, BG_OBJECT_FLAG2_EY_ENTRY, 2174.249756f, 1568.513185f, 1159.740235f, -1.503602f, 0, 0, 0.662620f, -0.748956f, RESPAWN_ONE_DAY)
        ...
        || !AddObject(BG_EY_OBJECT_FLAG_FEL_REAVER, BG_OBJECT_FLAG1_EY_ENTRY, 2044.28f, 1729.68f, 1189.85f, 0.017453f, 0, 0, 0.992546f, 0.121869f, RESPAWN_ONE_DAY)
        // tower cap
        || !AddObject(BG_EY_OBJECT_TOWER_CAP_FEL_REAVER, BG_OBJECT_FR_TOWER_CAP_EY_ENTRY, 2024.600708f, 1742.819580f, 1192.157715f, 2.443461f, 0, 0, 0.939693f, 0.342020f, RESPAWN_ONE_DAY)
        || !AddObject(BG_EY_OBJECT_TOWER_CAP_BLOOD_ELF, BG_OBJECT_BE_TOWER_CAP_EY_ENTRY, 2050.493164f, 1372.235962f, 1191.563477f, 1.710423f, 0, 0, 0.754710f, 0.656059f, RESPAWN_ONE_DAY)
        || !AddObject(BG_EY_OBJECT_TOWER_CAP_DRAENEI_RUINS, BG_OBJECT_DR_TOWER_CAP_EY_ENTRY, 2301.010498f, 1386.931641f, 1194.183472f, 1.570796f, 0, 0, 0.707107f, 0.707107f, RESPAWN_ONE_DAY)
        || !AddObject(BG_EY_OBJECT_TOWER_CAP_MAGE_TOWER, BG_OBJECT_HU_TOWER_CAP_EY_ENTRY, 2282.121582f, 1760.006958f, 1186.707153f, 1.919862f, 0, 0, 0.819152f, 0.573576f, RESPAWN_ONE_DAY)
)
    {
    ...
    ANTICHEAT_time_since_pull = 0;
    ANTICHEAT_original_Flag_Puller = 0;
    return true;
}
...
void BattlegroundEY::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    if (killer)
        ...
        else TC_LOG_ERROR("bg.battleground", "BattlegroundEY::HandleKillPlayer: Killer player not found");
    else TC_LOG_ERROR("bg.battleground", "BattlegroundEY::HandleKillPlayer: Victim player not found");
...
void BattlegroundEY::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
    ...
    if (m_FlagState == BG_EY_FLAG_STATE_ON_BASE) {
        ANTICHEAT_time_since_pull = 1;
        ANTICHEAT_original_Flag_Puller = Source->GetGUID();
    }
    if (m_FlagState == BG_EY_FLAG_STATE_ON_BASE)
...
void BattlegroundEY::EventPlayerCapturedFlag(Player* Source, uint32 BgObjectType)
{
    ...
    if ((ANTICHEAT_time_since_pull > 0) && (ANTICHEAT_time_since_pull < 12000) && (ANTICHEAT_original_Flag_Puller == Source->GetGUID())) //he cheated. too fast.
    {
        Source->LeaveBattleground();
        Source->CastCustomSpell(26013, SPELLVALUE_DURATION, 86400000, Source);
        Source->RemoveAurasDueToSpell(BG_EY_NETHERSTORM_FLAG_SPELL);
