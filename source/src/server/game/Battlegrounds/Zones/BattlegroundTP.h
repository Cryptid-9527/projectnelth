...
enum BG_TP_ObjectTypes
{
    ...
    BG_TP_OBJECT_alligate_z_forceChange,
    BG_TP_OBJECT_MAX
};
...
enum BG_TP_ObjectEntry
{
    ...
    BG_OBJECT_WUFC_SOCALLED_MASSIVE_DOOR = 180322
};
...
*/
    ...
    uint16 tp_door_removal_timer = 0;
    uint16 pickedUpTooSoon_counter = 0;
...
private:
    ...
    //Begin new ASSAULT mechanics
    uint8 ASSAULT_STACKS;
    uint8 ASSAULT_TICK_MINUTES;
    uint16 ASSAULT_TICKS_COUNT;
    void setup_ASSAULT_VALUES();
    uint16 get_ASSAULT_ID(bool wantCurrentDebuff);
    void remove_ASSAULTS(Player* target);
    void apply_correct_ASSAULT(Player* target);
    uint8 get_ASSAULT_STACKS() {
        return ASSAULT_STACKS;
    }

    void update_ALL_ASSAULTS();
    void remove_REMAINING_ASSAULTS();
    void clock_tick_ASSAULT_MANAGEMENT();
    //End new ASSAULT mechanics
    void reset_flag_in_GY();
    /// Flag Events
    ...
    /// Respawn flag method
    void RespawnFlag(uint32 team, uint8 respawnReason);

    /// Members:
    uint32 ANTICHEAT_time_since_pull[2]; ///< Stored timer for how long a flag has been away from it's base, standalone in-script anticheat measure.
    uint64 ANTICHEAT_original_Flag_Puller[2]; ///< Stored GUIDs of the "original flag carrier", used for standalone in-script anticheat measures (EX: _flagKeepers[TEAM_ALLIANCE] is guid for a horde player)
    ...
    bool _bothFlagsKept; ///< unreliable? I guess?  
