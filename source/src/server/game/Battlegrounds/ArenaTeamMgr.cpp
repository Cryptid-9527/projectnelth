...
void ArenaTeamMgr::LoadArenaTeams()
{
    ...
    CharacterDatabase.Execute("DELETE FROM arena_team_member WHERE `arena_team_member`.`arenaTeamId` NOT IN (SELECT `arena_team`.`arenaTeamId` FROM arena_team)");       // One-time query
    ...
    QueryResult result2 = CharacterDatabase.Query(
        ...
        " LEFT JOIN character_pvp_stats AS cas ON c.guid = cas.guid AND (cas.slot = 0 AND ate.type = 2 OR cas.slot = 1 AND ate.type = 3 OR cas.slot = 2 AND ate.type = 5)"
...
{
    for (auto titr = GetArenaTeamMapBegin(); titr != GetArenaTeamMapEnd(); ++titr)
        if (auto at = titr->second)
        {
            at->FinishWeek();
            at->SaveToDB(true);
