...
bool ScriptMgr::OnGossipHello(Player* player, Creature* creature, uint32 scriptId)
{
    if (!scriptId)
...
bool ScriptMgr::OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action, uint32 scriptId)
{
    if (!scriptId)
        scriptId = creature->GetScriptId();
...
bool ScriptMgr::OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code, uint32 scriptId)
{
    if (!scriptId)
        scriptId = creature->GetScriptId();
...
void ScriptMgr::BeforeGameObjectStateChanged(GameObject* go, uint32 state)
{
    ASSERT(go);

    GET_SCRIPT(GameObjectScript, go->GetScriptId(), tmpscript);
    tmpscript->BeforeGameObjectStateChanged(go, state);
}

void ScriptMgr::AfterGameObjectStateChanged(GameObject* go, uint32 state)
{
    ASSERT(go);

    GET_SCRIPT(GameObjectScript, go->GetScriptId(), tmpscript);
    tmpscript->AfterGameObjectStateChanged(go, state);
}
...
void ScriptMgr::OnPlayerItemEquipped(Player* player, Item* pItem, bool equip /*=true*/)
{
    FOREACH_SCRIPT(PlayerScript)->OnItemEquip(player, pItem, equip);
}
