...
struct LootStoreItem
{
    ...
    bool Roll(bool rate, Object* source, Loot* loot) const;                             // Checks if the entry takes it's chance (at loot generation)
...
struct Loot
{
    ...
    Loot(uint32 _gold = 0) : gold(_gold), unlootedCount(0), loot_source(nullptr), loot_type(LOOT_CORPSE), maxDuplicates(1), containerID(0) {}
    ~Loot() { clear(); }
    ...
    std::set<uint64> const* GetLooters();
    ...
    void generateMoneyLoot(uint32 minAmount, uint32 maxAmount, Object* center);
    bool FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError = false, uint16 lootMode = LOOT_MODE_DEFAULT);

    // Inserts the item into the loot (called by LootTemplate processors)
    void AddItem(LootStoreItem const & item);
    bool DropItem(Object* source, LootItem& loot);
