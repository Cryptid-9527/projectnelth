...
void InfoMgr::Initialize()
{
    ...
    start = getMSTime();
    count = 0;
    if (QueryResult mmrResult = CharacterDatabase.Query("SELECT guid, matchMakerRating, slot FROM character_pvp_stats WHERE guid != 0"))
    {
        do
        {
...
void InfoMgr::AddBGInfo(SlotStoreType Slot, uint32 instanceID, uint32 team1, uint32 team2, uint16 mmr1, uint16 mmr2, uint8 type)
{
