    ...
    _navMeshQuery(NULL)
{
    ...
    if (mapId == 607 && owner->HasUnitTypeMask(UNIT_MASK_MINION | UNIT_MASK_GUARDIAN | UNIT_MASK_CONTROLABLE_GUARDIAN | UNIT_MASK_HUNTER_PET | UNIT_MASK_PET))
    {
        float zDiff = fabs(owner->GetPositionZ() - (owner->getVictim() ? owner->getVictim()->GetPositionZ() : 0.00f));
        if (!owner->getVictim() || (!owner->IsFriendlyTo(owner->getVictim()) && zDiff <= 6.00f))
        {
            CreateFilter();
            return;
        }
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine, bool inverted)
{
    if (auto c = _sourceUnit->ToCreature())
        if (auto cTemplate = c->GetCreatureTemplate())
            if (cTemplate->flags_extra & CREATURE_FLAG_EXTRA_IGNORE_PATHFINDING)
                return false;
    ...
    SetEndPosition(inverted ? start : dest);
    SetStartPosition(inverted ? dest : start);
    ...
    if (!_navMesh || !_navMeshQuery || _sourceUnit->HasUnitState(UNIT_STATE_IGNORE_PATHFINDING) ||
        !HaveTile(inverted ? dest : start) || !HaveTile(inverted ? start : dest) || _sourceUnit->HasUnitState(UNIT_STATE_EVADE) /*|| _sourceUnit->GetTransport()*/)
    {
        BuildShortcut();
    ...
    BuildPolyPath(inverted ? dest : start, inverted ? start : dest);
    return true;
}
