...
void RandomMovementGenerator<Creature>::_setRandomLocation(Creature* creature)
{
    ...
    if (is_air_ok)                                          // 3D system above ground and above water (flying mode)
    {
        ...
        if (creature->CanFly())
            if (creature->GetPositionZ() > (map->GetWaterOrGroundLevel(destX, destY, creature->GetPositionZ())+1.f))
            if (!creature->HasByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND) || !creature->HasByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_HOVER))
                creature->SetByteValue(UNIT_FIELD_BYTES_1, 3, uint8(UNIT_BYTE1_FLAG_ALWAYS_FLYING));
    ...
    PathGenerator random_movement_path(creature);
    random_movement_path.SetPathLengthLimit(range * 8);
    
    if (!creature->IsWithinLOS(destX, destY, destZ) && !is_air_ok && random_movement_path.CalculatePath(destX, destY, destZ))
    {
        init.SetSmooth();
        init.MovebyPath(random_movement_path.GetPath(), 0);
        init.SetVelocity(creature->GetSpeed(MOVE_WALK));
        init.Launch();
    }
    else
    {
