    ...
    do
    {
        ...
        switch (type)
        {
            ...
            case DISABLE_TYPE_VMAP:
            {
                ...
                {
                    Tokenizer tokens3(params_0, ',');
                    for (uint8 i = 0; i < tokens3.size(); )
                        if (tokens3[i] != 0)
                        data.params[0].insert(atoi(tokens3[i++]));

                    Tokenizer tokens4(params_1, ',');
                    for (uint8 i = 0; i < tokens4.size(); )
                        if (tokens4[i] != 0)
                        data.params[1].insert(atoi(tokens4[i++]));
                }
...
bool IsDisabledFor(DisableType type, uint32 entry, Unit const* unit, uint8 flags)
{
    ...
    switch (type)
    {
        ...
        case DISABLE_TYPE_HEIGHT_RELOCATION:
            ...
            break;
        case DISABLE_TYPE_VMAP:
            if (flags & itr->second.flags)
            {
                if (flags == VMAP_DISABLE_LOS)
                {
                    auto ZoneIds { itr->second.params[0] };
                    auto areaIds { itr->second.params[1] };

                    if (unit)
                    {

                        if (unit->GetMapId() == entry)
                        {
                            if (ZoneIds.empty())                                            //Map match, empty zone specification
                            {
                                if (areaIds.empty())                                        //Map match, empty zone specification, empty area specification
                                {
                                    //TC_LOG_ERROR("sql.sql", "Returning 1");
                                    return true;
                                }

                                if (areaIds.find(unit->GetAreaId()) != areaIds.end())       //Map match, empty zone specification, area match
                                {
                                    //TC_LOG_ERROR("sql.sql", "Returning 2");
                                    return true;
                                }


                                //TC_LOG_ERROR("sql.sql", "Returning 3");
                                return false;                                               //Map match, zone specification is empty, an area is specified but not matching.
                            }
                            else                                                            //Map match, zones are specified
                            {

                                if (ZoneIds.find(unit->GetZoneId()) != ZoneIds.end())       //Map match, zone match
                                {
                                    if (areaIds.empty())                                    //Map match, zone match, empty area specification
                                    {
                                        //TC_LOG_ERROR("sql.sql", "Returning 4");
                                        return true;
                                    }

                                    if (areaIds.find(unit->GetAreaId()) != areaIds.end())   //Map match, zone match, area match
                                    {
                                        //TC_LOG_ERROR("sql.sql", "Returning 5");
                                        return true;
                                    }

                                    //TC_LOG_ERROR("sql.sql", "Returning 6");
                                    return false;                                           //Map match, zone match, area is specified but does not match.
                                }
                                else
                                {
                                    if (areaIds.find(unit->GetAreaId()) != areaIds.end())   //Map match, zone match, area match
                                    {
                                        TC_LOG_FATAL("sql.sql", "DisableMgr::IsDisabledFor - DISABLE_TYPE_VMAP flag VMAP_DISABLE_LOS invalid ZONE/AREA pair recognized: ZONE: %u, AREA: %u",
                                            unit->GetZoneId(), unit->GetAreaId());
                                        //TC_LOG_ERROR("sql.sql", "Returning 7");
                                        return false;
                                    }
                                    //TC_LOG_ERROR("sql.sql", "Returning 8");
                                    return false;                                           //Map match, Zone is specified but does not match
                                }
                                //TC_LOG_ERROR("sql.sql", "Returning 9");
                                return false;
                            }
                            //TC_LOG_ERROR("sql.sql", "Returning 10");
                            return false;
                        }
                        //TC_LOG_ERROR("sql.sql", "Returning 11");
                        return false;
                    }
                    ////TC_LOG_ERROR("sql.sql", "Returning 12");
                    return false;                                                   //Map does not match.
                }
                ////TC_LOG_ERROR("sql.sql", "Returning 13");
                return true;
            }
    }
    return false;
