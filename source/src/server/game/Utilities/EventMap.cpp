...
{
	uint64 eventId64 = eventId;
    if (group && group < 16)
        eventId64 |= (1LL << (group + 31));

    if (phase && phase < 16)
        eventId64 |= (1LL << (phase + 47));

    _eventMap.insert(EventStore::value_type(_time + time, eventId64));
}
...
uint32 EventMap::ExecuteEvent()
{
    while (!Empty())
    {
        ...
        if (itr->first > _time)
            return 0;
        else if (_phase && (itr->second & PHASE_MASK) && !((itr->second >> 48) & _phase))
            _eventMap.erase(itr);
        else
        {
