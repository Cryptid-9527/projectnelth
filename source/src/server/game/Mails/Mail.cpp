...
void MailDraft::SendMailTo(SQLTransaction& trans, MailReceiver const& receiver, MailSender const& sender, MailCheckMask checked, uint32 deliver_delay)
{
    ...
    /*
        GM Command History Logging
    */
    auto mail_log{ CharacterDatabase.GetPreparedStatement(CHAR_INS_HISTORY_MAIL) };
    //mail_log->setUInt64(0, 0);// filled by default
    mail_log->setUInt64(1, sender.GetSenderId());
    mail_log->setUInt64(2, receiver.GetPlayerGUIDLow());
    mail_log->setUInt64(3, m_money);
    mail_log->setUInt64(4, m_COD);

    uint8 index_log { 5 };
    for (auto i = 0; i < 12; ++i)
    {
        auto index_init{ 5 + (i * 3) };
        mail_log->setUInt32(uint8(index_init), 0);
        mail_log->setUInt64(uint8(index_init+1), 0);
        mail_log->setUInt64(uint8(index_init+2), 0);
    }
    ...
    for (MailItemMap::const_iterator mailItemIter = m_items.begin(); mailItemIter != m_items.end(); ++mailItemIter)
    {
        ...
        mail_log->setUInt32(uint8(index_log),       pItem->GetEntry());
        index_log++;
        mail_log->setUInt64(uint8(index_log),   pItem->GetGUIDLow());
        index_log++;
        mail_log->setUInt64(uint8(index_log),   pItem->GetCount());
        index_log++;
    }

    if (sender.GetSenderId() != receiver.GetPlayerGUIDLow())
    trans->Append(mail_log);
