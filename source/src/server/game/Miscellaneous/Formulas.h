        ...
        inline uint32 Gain(Player* player, Unit* u)
        {
            ...
            else
            {
                ...
                if (player->GetCustomXpRate())
                    gain = uint32(gain * player->GetCustomXpRate());
                else
                ...
                /*
                    - Reported by Dacha
                    https://www.reddit.com/r/wow/comments/k732z/til_rare_elite_creatures_give_a_lot_more_xp_than/
                    - "Rare Mobs should give xp similar to a quest"

                    https://wowwiki-archive.fandom.com/wiki/Formulas:Mob_XP
                    - "Elites = xp * 2"
                */
                if (auto c = u->ToCreature())
                    if (auto temp = c->GetCreatureTemplate())
                    {
                        if (temp->rank == 1)//"Elite"
                            gain *= 2;
