...
enum SpellAttr6
{
    ...
    SPELL_ATTR6_AUTOATTACK_STOP                  = 0x00800000, // 23 its break autoattack
...
enum SpellAttr10
{
    ...
    SPELL_ATTR10_IGNORE_RESILIENCE                = 0x00000002, //  1  - Ignore resilience
...
enum BattlegroundQueueTypeId
{
    ...
    BATTLEGROUND_QUEUE_RBG_10_VS_10 = 15,
    BATTLEGROUND_QUEUE_RBG_15_VS_15 = 16,
    BATTLEGROUND_QUEUE_RBG_25_VS_25 = 17,
...
enum PvP_Detection_Data_Events
{
    //kickbotting
    PVP_DETECTION_EVENT_KICK_ATTEMPT    = 1,
    //spell placement
    PVP_DETECTION_EVENT_SPELL_PLACEMENT = 2,
};

#define NORTH       (0.00f)
#define NORTH_EAST  (M_PI * 1.75f)
#define EAST        (M_PI * 1.50f)
#define SOUTH_EAST  (M_PI * 1.25f)
#define SOUTH       (M_PI * 1.00f)
#define SOUTH_WEST  (M_PI * 0.75f)
#define WEST        (M_PI * 0.50f)
#define NORTH_WEST  (M_PI * 0.25f)

#define MONEY_GOLD_RATIO 10000//money / this number = gold

enum MuteStates
{
    MUTE_STATE_UNMUTED          = 0,
    MUTE_STATE_NORMAL_MUTE      = 1,
    MUTE_STATE_SHADOW_MUTE      = 2,
    MUTE_STATE_GM_FROZEN        = 3,
    MUTE_STATE_LIMIT
};

enum CarbonCopyFlags
{
    CARBON_COPY_NOTHING                 = 0,// (Disabled)
    CARBON_COPY_PUBLIC_CHANNELS         = 1,// - (+Channel chats such as localdefense, world, lookingforgroup)
    CARBON_COPY_YELL_TEXT               = 2,// - (+Yells)
    CARBON_COPY_SAY_TEXT                = 4,// - (+Say)
    CARBON_COPY_RAID_TEXT               = 8,// - (+Raid / Battleground)
    CARBON_COPY_PARTY_TEXT              = 16,// - (+Party)
    CARBON_COPY_GUILD_TEXT              = 32,// - (+Guild)
    CARBON_COPY_OFFICER_TEXT            = 64,// - (+Guild Officer)
    CARBON_COPY_PRIVATE_WHISPERS        = 128,// - (+Whispers, only recommended for situations where an active dm advertisement campaign is ongoing)
};

enum PlayerComplaintTypes
{
    PLAYER_COMPLAINT_CHAT_SPAM          = 0,// right click on chat, report for spam
    PLAYER_COMPLAINT_CHEATING           = 1,//right click on pic, report for cheating
    PLAYER_COMPLAINT_UNK_2              = 2,//unk
    PLAYER_COMPLAINT_BAD_CHAR_NAME      = 3,//right click on pic, bad player name
    PLAYER_COMPLAINT_UNK_4              = 4,//unk
    PLAYER_COMPLAINT_UNK_5              = 5,//unk
    PLAYER_COMPLAINT_BAD_GUILD_NAME     = 6,//right click on pic, bad guild name
    PLAYER_COMPLAINT_UNK_7              = 7,//unk
    PLAYER_COMPLAINT_UNK_8              = 8,//unk
    PLAYER_COMPLAINT_BAD_ARENA_NAME     = 9,//right click on pic, bad arena team name
    PLAYER_COMPLAINT_CHAT_BAD_LANG      = 10,//right click on chat, report for language
};

enum AuctionHouseFloodMitigations
{
    AH_FLOOD_LIMIT_DISABLED     = 0,

    AH_FLOOD_LIMIT_BY_ANY_STACK_OF_ITEM_FROM_PLAYER = 1,
    AH_FLOOD_LIMIT_BY_SPECIFIC_STACK_OF_ITEM_FROM_PLAYER = 2,
};

enum PlayerFloodResponses   //An influx of new players brings unique temporary demands for changes
{
    FLOOD_RESPONSE_RESPAWN      = 0,
    FLOOD_RESPONSE_LOOT_MONEY,
    FLOOD_RESPONSE_LOOT_QUEST,
    FLOOD_RESPONSE_LOOT_NONQUEST,
};

enum AccountAccessFlags //Bitmask for special account access that should not be linear
{
    ACCOUNT_FLAG_DEVELOPER                      = 0,     //When typing .gm, will instead display <Dev> over <GM>
    ACCOUNT_FLAG_COMMENTATOR,                               //When typing .gm, will instead display <Comm> over <GM>
    ACCOUNT_FLAG_HEAD_GAMEMASTER,
    ACCOUNT_FLAG_UNK_3,                       //Account is subject to rewards for their contribution to creating the server
    ACCOUNT_FLAG_GUEST_ACCESS_ONLY,
    ACCOUNT_FLAG_GUEST_ACCESS_CONTROLLER,
    ACCOUNT_FLAG_7,
    ACCOUNT_FLAG_8,
    ACCOUNT_FLAG_9,
    ACCOUNT_FLAG_10,
...
enum AccountPremiumPerks
{
    ACCOUNT_PERK_FULL_STATUS                = 0,
    ACCOUNT_PERK_50_CHARACTERS,
    ACCOUNT_PERK_PROJECT_CONTRIBUTOR,
    ACCOUNT_PERK_EARLY_OPEN_BETA_TESTER,
