...
World::World()
{
    ...
    m_entranceSecurityLevel = SEC_PLAYER;
...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_bool_configs[CONFIG_RATE_ADJUSTMENT_FOR_PLAYER_FLOOD] = sConfigMgr->GetBoolDefault("Rate.PlayerFloodResponse.Enable", false);
    ...
    m_int_configs[CONFIG_LAYER_CAP]                       = sConfigMgr->GetIntDefault("Layering.ExtraLayers", 0);
    ASSERT(m_int_configs[CONFIG_LAYER_CAP] < 32);
    ...
    m_bool_configs[CONFIG_SOLOQ_CAST_DESERTER]                       = sConfigMgr->GetBoolDefault("SoloQueue.CastDeserter", true);
	m_bool_configs[CONFIG_DUNGEON_CAST_DESERTER]					 = sConfigMgr->GetBoolDefault("Dungeon.CastDeserter", true);
    ...
    m_bool_configs[CONFIG_SOLO_QUEUE_DPS_RESTRICTION]                = sConfigMgr->GetBoolDefault("SoloQueue.Dps.Restriction", true);
    m_bool_configs[CONFIG_SOLO_QUEUE_ENABLE_MMR]                     = sConfigMgr->GetBoolDefault("SoloQueue.Enable.Mmr", true);
    m_bool_configs[CONFIG_ARENA_DAMPENING_ENABLED]                   = sConfigMgr->GetBoolDefault("Arena.Dampening.Enable", false);
    m_int_configs[CONFIG_ARENA_DAMPENING_BRACKETS]                   = sConfigMgr->GetIntDefault("Arena.Dampening.Brackets", 0);
    m_int_configs[CONFIG_ARENA_DAMPENING_STARTTIME]                  = sConfigMgr->GetIntDefault("Arena.Dampening.StartTime", 5);
    m_int_configs[CONFIG_ARENA_DAMPENING_INTERVAL]                   = sConfigMgr->GetIntDefault("Arena.Dampening.Interval", 10);
    m_bool_configs[CONFIG_ARENA_LOG_EXTENDED_INFO]                   = sConfigMgr->GetBoolDefault("ArenaLog.ExtendedInfo", false);
    m_float_configs[CONFIG_ARENA_WIN_RATING_MODIFIER_1]              = sConfigMgr->GetFloatDefault("Arena.ArenaWinRatingModifier1", 96.0f);
    m_float_configs[CONFIG_ARENA_WIN_RATING_MODIFIER_2]              = sConfigMgr->GetFloatDefault("Arena.ArenaWinRatingModifier2", 48.0f);
    m_float_configs[CONFIG_ARENA_LOSE_RATING_MODIFIER]               = sConfigMgr->GetFloatDefault("Arena.ArenaLoseRatingModifier", 48.0f);
    m_float_configs[CONFIG_ARENA_MATCHMAKER_RATING_MODIFIER]         = sConfigMgr->GetFloatDefault("Arena.ArenaMatchmakerRatingModifier", 48.0f);
    ...
    m_bool_configs[CONFIG_TOURNAMENT_ENABLE] = sConfigMgr->GetBoolDefault("Tournament.Mode", false);
    m_int_configs[CONFIG_TOURNAMENT_VENDOR] = sConfigMgr->GetIntDefault("TournamentVendor.Season", 11);
    m_bool_configs[CONFIG_TOURNAMENT_SET_2_ENABLE] = sConfigMgr->GetBoolDefault("TournamentVendor.EnableSet2", true);
    m_bool_configs[CONFIG_TOURNAMENT_NO_FLY] = sConfigMgr->GetBoolDefault("Tournament.NoFly", false);
    m_bool_configs[CONFIG_TOURNAMENT_RESS_ON_DEATH] = sConfigMgr->GetBoolDefault("Tournament.RessDead", false);
    ...
    m_bool_configs[CONFIG_GM_REQUIRE_BADGE_FOR_CHAT] = sConfigMgr->GetBoolDefault("GM.RequireBadgeForChat", false);

    // Cryptid carbon copy
    m_int_configs[CONFIG_CARBON_COPY] = sConfigMgr->GetIntDefault("CarbonCopy.Bitmask", 0);

    // Spell Delay system
    m_bool_configs[CONFIG_SPELL_DELAY_ENABLE] = sConfigMgr->GetBoolDefault("Spell.Delay.Enable", false);
    m_int_configs[CONFIG_SPELL_DELAY_SPELLS] = sConfigMgr->GetIntDefault("Spell.Delay.Spells", 100);
    m_int_configs[CONFIG_SPELL_DELAY_SLOWS] = sConfigMgr->GetIntDefault("Spell.Delay.Slows", 200);

    // Morpher
    m_int_configs[CONFIG_MORPHER_NPC_COST] = sConfigMgr->GetIntDefault("Morpher.Npc.Cost", 0);

    /*
        Additional blocks to prevent staff accounts from claiming achievements.
        Disable if required for development.
    */
    m_bool_configs[CONFIG_REALMFIRST_BLOCK_FOR_STAFF] = sConfigMgr->GetBoolDefault("Realmfirst.BlockForStaff", true);

    //Auction house flood controlled visibility
    m_int_configs[CONFIG_AUCTION_FLOOD_CONTROL_TYPE] = sConfigMgr->GetIntDefault("Auction.FloodControl.Type", 0);
    m_int_configs[CONFIG_AUCTION_FLOOD_CONTROL_VALUE] = sConfigMgr->GetIntDefault("Auction.FloodControl.Count", 0);
...
void World::SetInitialWorldSettings()
{
    ...
    sChallengeModeMgr->LoadChallengeData();
    sChallengeModeMgr->LoadMultipliersData();
    ...
    TC_LOG_INFO("misc", "Loading Template Talent Trees...");
    sTemplateMgr->LoadTalentTrees();

    // Load templates for Template NPC #2
    TC_LOG_INFO("misc", "Loading NPC Templates...");
    sTemplateMgr->LoadNpcTemplates();

    TC_LOG_INFO("misc", "Loading Taxi Node Levels...");
    sMapMgr->InitNodeLevels();

    TC_LOG_INFO("misc", "Loading Mythic+ Affix data...");
    InitAffixData();
...
void World::SendAntiCheat(WorldSession* s, int32 string_id, ...)
{
    if (!s->AntiCheatNotificationRequest())
        return;
...
BanReturn World::BanAccount(BanMode mode, std::string const& nameOrIP, std::string const& duration, std::string const& reason, std::string const& author)
{
    ...
    switch (mode)
    {
        case BAN_IP:
        {
            ...
            PreparedStatement* pre_stmt2 = LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_MASKING_IP_FROM_MASK);
            pre_stmt2->setString(0, std::string(nameOrIP));
            std::string real_ip{ "" };
            if (auto pre_result = LoginDatabase.Query(pre_stmt2))
                real_ip = (*pre_result)[0].GetString();

            stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_BY_IP);
            stmt->setString(0, real_ip);
    ...
    do
    {
        ...
        AcknowledgePlayerComplaintsForBanAccountWide(account, 0);
...
bool World::RemoveBanAccount(BanMode mode, std::string const& nameOrIP)
{
    ...
    if (mode == BAN_IP)
    {
        ...
        stmt->setString(0, real_ip);
        //TC_LOG_ERROR("sql.sql", "Unban IP executing: %s", real_ip);
        LoginDatabase.Execute(stmt);
...
BanReturn World::BanCharacter(std::string const& name, std::string const& duration, std::string const& reason, std::string const& author)
{
    ...
    AcknowledgePlayerComplaintsForBan(guid, 0);
    return BAN_SUCCESS;
}
...
void World::AcknowledgePlayerComplaintsForBan(uint64 banned_guid, uint64 banning_gm)
{
    if (auto reporter_list = CharacterDatabase.PQuery("SELECT DISTINCT source_guid as GUID FROM character_complaints cc WHERE cc.target_guid = %u", banned_guid))
    {
        SQLTransaction trans = CharacterDatabase.BeginTransaction();
        do
        {
            if (auto fields2 = reporter_list->Fetch())
            {
                auto source_guid{ fields2[0].GetUInt64() };
                std::string inputName{ "player" };
                sObjectMgr->GetPlayerNameByGUID(source_guid, inputName);

                auto name{ inputName };

                if (auto count_and_recentDate = CharacterDatabase.PQuery("SELECT SUM(COUNT), MAX(DATETIME) FROM ((SELECT COUNT, DateTime FROM character_complaints WHERE source_guid = %u AND target_guid = %u)) cc;", source_guid, banned_guid))
                    if (auto fields_CAR = count_and_recentDate->Fetch())
                    {
                        auto count{ fields_CAR[0].GetUInt32() };
                        auto date{ fields_CAR[1].GetCString() };

                        auto mailId = sObjectMgr->GenerateMailID();
                        std::ostringstream message_stream{ "" };
                        message_stream << name <<",\n\nThis message is a notification of action against a player you reported ";
                        if (count > 1)
                            message_stream << count << " times, most recently on ";
                        else
                            message_stream << "on ";
                        message_stream << date << " server time.\n\nWe thank you for your efforts in helping us maintain a clean server.\n\nNeltharion GM Team";

                        std::string title{ "Thank You" };
                        std::string pre_body{ message_stream.str() };
                        std::string body{ pre_body.c_str() };
                        ...
                        stmt->setInt8(++index, int8(MAIL_STATIONERY_GM));
                        stmt->setUInt16(++index, 0);
                        stmt->setUInt32(++index, 0);
                        stmt->setUInt32(++index, source_guid);
                        stmt->setString(++index, title);
                        stmt->setString(++index, body);
                        stmt->setBool(++index, false);
                        stmt->setUInt64(++index, uint64(time(NULL)) + (7 * uint64(DAY)));
                        stmt->setUInt64(++index, uint64(time(NULL)));
                        stmt->setUInt64(++index, 0);
                        stmt->setUInt32(++index, 0);
                        stmt->setUInt8(++index, uint8(MAIL_CHECK_MASK_HAS_BODY));
                        trans->Append(stmt);

                    }
            }
        } while (reporter_list->NextRow());

        CharacterDatabase.CommitTransaction(trans);
    }
}

void World::AcknowledgePlayerComplaintsForBanAccountWide(uint64 banned_account, uint64 banning_gm)
{
    if (auto target_account_guid_list = CharacterDatabase.PQuery("SELECT guid FROM characters WHERE account = %u", banned_account))
    {
        do
        {
            if (auto results = target_account_guid_list->Fetch())
                if (auto character_guid{ results[0].GetUInt64() })
                {
                    AcknowledgePlayerComplaintsForBan(character_guid, banning_gm);
                    CharacterDatabase.PQuery("DELETE FROM character_complaints WHERE target_guid = %u", character_guid);
                }
        } while
            (target_account_guid_list->NextRow());
    }
}
...
void World::ShutdownMsg(bool show, Player* player, const std::string& reason)
{
    ...
    if (show ||
		(m_ShutdownTimer < 11 * SECOND && (m_ShutdownTimer % 1) == 0) || // < 11 sec; every sec
...
void World::InitAffixData()
{
    if (auto stmt = WorldDatabase.GetPreparedStatement(WORLD_SEL_AFFIX_TEMPLATES))
        if (auto results = WorldDatabase.Query(stmt))
        {
            uint32 affix_count{ 0 };
            do
            {
                if (auto fields2 = results->Fetch())
                {
                    uint8 affix{ fields2[0].GetUInt8() };
                    std::string name{ fields2[1].GetString() };
                    std::string icon{ fields2[2].GetString() };
                    std::string description{ fields2[3].GetString() };

                    m_AffixData.insert({ affix, {name, icon, description} });
                    if (affix > 0)
                        affix_count++;
                }
            ...
            TC_LOG_INFO("misc", "Loaded %u Mythic+ Affixes.", affix_count);
        }
}

AffixDetails const* World::GetAffixDetails(uint8 affix)
{
    auto entry = m_AffixData.find(uint32(affix));
    if (entry != m_AffixData.end())
        return &(entry->second);
...
void World::LoadDBAllowedSecurityLevel()
{
    ...
    if (result)
        if (auto field = result->Fetch())
        {
            auto level_charselectScreen = field[0].GetUInt8();
            auto level_enterWorld       = field[1].GetUInt8();
            SetPlayerSecurityLimit(AccountTypes(level_charselectScreen));
            SetEntranceSecurityLimit(AccountTypes(level_enterWorld));
        }
...
void World::SetEntranceSecurityLimit(AccountTypes _sec)
{
    AccountTypes sec = _sec < SEC_CONSOLE ? _sec : SEC_PLAYER;
    bool update = sec > m_entranceSecurityLevel;
    m_entranceSecurityLevel = sec;
    if (update)
        KickAllLess(m_entranceSecurityLevel);
}
...
void World::InitPacketThrottling()
{
    ...
    /*
    These first three are intended to minimize the potential of any sort of vendor duping.
    */
    opcodePerSecond[CMSG_BUY_ITEM] = 5;
    opcodePerSecond[CMSG_BUYBACK_ITEM] = 5;
    opcodePerSecond[CMSG_SELL_ITEM] = 5;
    opcodePerSecond[CMSG_SUBMIT_COMPLAIN] = 1;
    opcodePerSecond[CMSG_MESSAGECHAT_GUILD] = 25;
    opcodePerSecond[CMSG_MESSAGECHAT_OFFICER] = 25;
    opcodePerSecond[CMSG_MESSAGECHAT_PARTY] = 25;
    opcodePerSecond[CMSG_MESSAGECHAT_RAID] = 25;
    opcodePerSecond[CMSG_MESSAGECHAT_RAID_WARNING] = 15;
    opcodePerSecond[CMSG_MESSAGECHAT_SAY] = 15;
    opcodePerSecond[CMSG_MESSAGECHAT_DND] = 15;
    opcodePerSecond[CMSG_MESSAGECHAT_WHISPER] = 3;
    opcodePerSecond[CMSG_MESSAGECHAT_YELL] = 3;
    opcodePerSecond[CMSG_MESSAGECHAT_EMOTE] = 3;
