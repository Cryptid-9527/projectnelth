...
namespace Trinity
{
    ...
    class AnyUnfriendlyDeadUnitInObjectRangeCheck
    {
    public:
        AnyUnfriendlyDeadUnitInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range) : i_obj(obj), i_funit(funit), i_range(range) {}
        bool operator()(Unit* u)
        {
            if (u->isDead() && i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u))
                return true;
            else
                return false;
        }
    ...
    class AnyUnfriendlyNoTotemUnitInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Unit* u)
            {
                ...
                if (!u->isTargetableForAttack(false, (i_obj->GetTypeId() == TYPEID_PLAYER)))
                    return false;
    ...
    {
        public:
            AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range) : i_obj(obj), i_funit(funit), i_range(range) {}
            bool operator()(Unit* u)
            {
                if (!u)
                    return false;
    ...
    {
        public:
            AnyFriendlyUnitInObjectRangeCheck(WorldObject const* obj, Unit* funit, float range, ConditionList* condList = NULL, bool playerOnly = false) :
                i_obj(obj), i_funit(funit), i_range(range), _condList(condList), i_playerOnly(playerOnly)
            {
                if (condList)
    ...
    class NearestAttackableUnitInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Unit* u)
            {
                if (u->isTargetableForAttack(true, (i_obj->GetTypeId() == TYPEID_PLAYER)) && i_obj->IsWithinDistInMap(u, i_range) &&
    ...
    */
    ...
    class NearestCreatureDistinctEntryWithLiveStateInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Creature* u)
            {
                if ((!i_entry || u->GetEntry() == i_entry) && u->isAlive() == i_alive && i_obj.IsWithinDistInMap(u, i_range) && u->GetGUID() != i_obj.GetGUID())
                {
                    i_range = i_obj.GetDistance(u);         // use found unit range as new range limit for next check
                    return true;
                }
