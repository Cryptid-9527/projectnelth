...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    uint8 muteType = fields[8].GetUInt8();
    ...
    uint32 recruiter = fields[10].GetUInt32();
    ...
    stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_ACCOUNT_ACCESS_FLAGS);

    stmt->setUInt32(0, id);
    stmt->setUInt32(1, sWorld->getRealmID());
    result = LoginDatabase.Query(stmt);

    uint32 accountFlags {0};
    if (result)
        if (auto res = result->Fetch())
            accountFlags = res[0].GetUInt32();

    // NOTE ATM the socket is single-threaded, have this in mind ...
    ACE_NEW_RETURN(m_Session, WorldSession(id, this, AccountTypes(security), expansion, mutetime, muteType, locale, recruiter, isRecruiter, accountFlags), -1);

    m_Crypt.Init(&k);

    m_Session->ReadAddonsInfo(addonsData);

    m_Session->SetOperatingSystem(os);
