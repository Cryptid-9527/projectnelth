...
bool VerificationMgr::IsValidPlayerSpell(Player* player, uint32 spellId)
{
    ...
    if ((spellInfo->AttributesEx7 & SPELL_ATTR7_HORDE_ONLY) && player->GetTeam() != HORDE && player->IsPlayingNative()) //Don't want to make ppl lose spells just by entering crossfaction BGs
    {
        valid &= false;
    ...
    if ((spellInfo->AttributesEx7 & SPELL_ATTR7_ALLIANCE_ONLY) && player->GetTeam() != ALLIANCE && player->IsPlayingNative()) //Don't want to make ppl lose spells just by entering crossfaction BGs
    {
        valid &= false;
