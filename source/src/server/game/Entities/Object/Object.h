...
{
public:
    typedef std::map<uint64, uint8*> FormatMaskMap;

    ExplodedGuid() : m_guidBytes(NULL), m_packet(NULL), _count(0), _rpos(0), _wpos(0), _maskrpos(0), _maskwpos(0) {}
    ExplodedGuid(uint8 count, ByteBuffer* packet);
    ExplodedGuid(uint8 count, const char* maskFormat, const char* storageFormat, ByteBuffer* packet);
    void Initialize(uint8 count, const char* maskFormat, const char* storageFormat, ByteBuffer* packet);


    void AddGuid(uint64 guid, uint8 idx = 0);

    ~ExplodedGuid();

    void SetFormatStrings(const char* maskFormat, const char* storageFormat)
    {
        CreateFormatMaps(maskFormat, storageFormat);
    }

    static void FreeFormatMasks();

    void ResetPositions() { _rpos = 0; _wpos = 0; _maskrpos = 0; _maskwpos = 0; }
    ...
    void ReadMaskCount(uint8 count);
    void WriteMaskCount(uint8 count);

    void ConditionalReadUint8();
    void ConditionalWriteUint8(ByteBuffer* packet = NULL);

    void ConditionalRead(uint8 count);
    void ConditionalWrite(uint8 count, ByteBuffer* packet = NULL);

    void SetStorage(ByteBuffer* b) { m_packet = b; }

    uint64 GetRaw(uint8 idx);
    ObjectGuid GetObjectGuid(uint8 idx);

    void SetCount(uint8 count) { _count = count; }

private:
    ExplodedGuid(const char* maskFormat, const char* storageFormat, ByteBuffer* packet);
    ExplodedGuid(ExplodedGuid const&);
    void CreateFormatMaps(const char* maskFormat, const char* storageFormat);
    void SetGuid(uint64 guid, uint8 idx);
...
class Object
{
    public:
        ...
        void SetFlag_Force(uint16 index, uint32 newFlag);//dev purposes only
...
//public:
    ...
    float GetRelativeAngleOffset(Unit* target) const;
    float GetRelativeAngleOffset(const Position* target) const;
    float GetRelativeAngleOffset(float x, float y) const;
...
class WorldObject : public Object, public WorldLocation
{
    ...
    public:
        ...
        void GetNearPoint(WorldObject const* searcher, float& x, float& y, float& z, float searcher_size, float distance2d, float absAngle, float controlZ = 0.f) const;
        void GetClosePoint(float& x, float& y, float& z, float size, float distance2d = 0, float angle = 0, float controlZ = 0.f) const
        {
            // angle calculated from current orientation
            GetNearPoint(NULL, x, y, z, size, distance2d, GetOrientation() + angle, controlZ);
        }
        ...
        virtual void SetLayerMask(uint32 newLayerMask, bool update, Map* new_map);
        uint32 GetLayerMask() const { return m_layerMask; }
        void SetLayerSourceMap(uint32 new_map_entry) {};
        virtual uint32 GetProperLayerMask();
        bool InSameLayer(WorldObject const* obj) const;
        bool InSameLayer(uint32 layermask) const;
        ...
        float GetDistance(const WorldObject* obj) const
        {
            ...
            //Position compare{ obj->GetPositionX(), obj->GetPositionY(), obj->GetPositionZ(), obj->GetOrientation() };
        ...
        uint64 GetIsolatedRevealTo() const
        { 
            return uint64(guid_revealed_to);
        };
        void SetIsolatedRevealTo(uint64 guid_who_may_detect) { guid_revealed_to = guid_who_may_detect; };
        ...
        void CalculateObjPassengerOffset(float& x, float& y, float& z, float& o);
        // Low Level Packets
        ...
        virtual void SetLayeringMap(Map* map);
        Map* FindLayeringMap() const { return m_layerMap; }
        Map* GetLayeringMap() const { ASSERT(m_currMap); return m_currMap; }
        ...
        std::list<Unit*> GetFriendlyUnitsInRange(float range, Unit* center) const;
        ...
        //LinearTransport* GetLinearTransport() const { return m_lineartransport; }
        Transport* GetLinearTransport() const { return m_transport; }
        ...
        void SetLinearTransport(Transport* t) { m_transport = t; }
    ...
    private:
        ...
        Map* m_layerMap;                                    //current object's Map location

        uint64 guid_revealed_to{ 0 };
        //uint32 m_mapId;                                     // object at map with map_id
        uint32 m_InstanceId;                                // in map copy with instance id
        uint32 m_phaseMask;                                 // in area phase state
        uint32 m_layerMask  {0};                                 // in area phase state
        uint32 m_layerSourceMap { 0 };                                 // Which map is our layerMask Based out of?
        ...
        { 
            ...
            if (GetMap() == obj->GetMap())  //check same map
                if (InSamePhase(obj))   //check same phase
                    if (InSameLayer(obj))   //check same phase
                    return false;       //These two things could potentially see each other
