...
void PlayerSocial::SendSocialList(Player* player)
{
    ...
    for (auto itr = m_playerSocialMap.begin(); itr != m_playerSocialMap.end(); ++itr)
    {
...
void SocialMgr::GetFriendInfo(Player* player, uint32 friendGUID, FriendInfo &friendInfo)
{
    ...
    if (pFriend)
        ...
        (
            (   
                pFriend->GetOTeam() == team || allowTwoSideWhoList  //Teams are okay to interact
            ) 
        ...
        pFriend->IsVisibleGloballyFor(player))
    {
                ...
                if (!pFriend->isGMVisible())
                if (auto s = pFriend->GetSession())
                if (s->HasAccessFlag(ACCOUNT_FLAG_HEAD_GAMEMASTER) && pFriend->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DND))
                    return;
