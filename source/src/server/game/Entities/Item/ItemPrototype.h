...
enum ItemFieldFlags
{
    ...
    ITEM_FLAG_ENCHANT_HIDDEN   = 0x04000000, // ?
...
struct ItemTemplate
{
    ...
    std::string GetQualityColorCode(bool include_cff) const
    {
        std::string result{ "" };
        ...
        case ITEM_QUALITY_POOR:
            result.append("9d9d9d");
            break;
        case ITEM_QUALITY_UNCOMMON:
            result.append("1eff00");
            break;
        case ITEM_QUALITY_HEIRLOOM:
            result.append("00ccff");
            break;
        case ITEM_QUALITY_ARTIFACT:
            result.append("e6cc80");
            break;
        case ITEM_QUALITY_RARE:
            result.append("0070dd");
            break;
        case ITEM_QUALITY_EPIC:
            result.append("a335ee");
            break;
        case ITEM_QUALITY_LEGENDARY:
            result.append("ff8000");
            break;
    ...
    std::string GetItemLink(bool for_gossip_interface, uint32 enchant_1, uint32 enchant_2, uint32 enchant_3, uint32 enchant_4, uint32 enchant_5, uint32 enchant_6, uint32 enchant_7) const
    {
        std::ostringstream link{ "" };

        //|cffffffff| Hitem: % d : 0 : 0 : 0 : 0 : 0 : 0 : 0 : 0 | h[% s] | h | r

        if (for_gossip_interface)
        {
            link << "|"
                << GetQualityColorCode(true)
                << "["
                << Name1
                << "]|r";
        }
