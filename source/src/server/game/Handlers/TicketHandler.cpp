...
void WorldSession::HandleSubmitComplainOpcode(WorldPacket & recvData)
{
    ...
    const char* complaint_description{ "" };

     //TC_LOG_ERROR("sql.sql","CMSG_SUBMIT_COMPLAIN:: count:%u guid:%u option:%u length :%u;posY:%f ,posX:%f ,posZ:%f ,MAP:%u ,poso:%f  text:%s",count,uint64(guid),options,length,posY,posX,posZ,mapID,posO,text.c_str());

    /*
        Check that the complained guid actually exists as a player
    */
    if (auto submitter = GetPlayer())
        if (auto target = ObjectAccessor::GetPlayer(*submitter, uint64(guid)))
            if (auto stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_COMPLAINT_LOG))
            {
                stmt->setUInt64(0, submitter->GetGUID());
                stmt->setUInt64(1, uint64(guid));
                stmt->setUInt32(2, options);
                stmt->setUInt32(3, (count ? count : 1));//we want this to be at least 1 to be counted correctly in the DB
                stmt->setUInt32(4, length);
                stmt->setUInt32(5, mapID);
                stmt->setFloat(6, posX);
                stmt->setFloat(7, posY);
                stmt->setFloat(8, posZ);
                stmt->setFloat(9, posO);
