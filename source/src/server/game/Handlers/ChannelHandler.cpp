...
void WorldSession::HandleJoinChannel(WorldPacket& recvPacket)
{
    ...
    if (char* c_name_unedited = (char*)channelName.c_str())
    if (char* c_name = (char*)channelName.c_str())
    {
        if (strlen(c_name) > 0)
            for (int x = 0; x < strlen(c_name); x++)
                c_name[x] = tolower(c_name[x]);
        char* name1 = "global";
        char* name2 = "world";
        ...
        */
        ...
        if (c_name)
        if (
            (strcmp(c_name, name1) == 0) 
            || 
            (strcmp(c_name, name2) == 0 && strcmp(c_name_unedited, name2) != 0)
            )
        {
            /*
            TC_LOG_ERROR("sql.sql", "global/world recognized: channel name is %s", c_name);
            channelName = std::string("world");
            channelLength = 5;
            */
            if (auto p = GetPlayer())
                ChatHandler(p->GetSession()).PSendSysMessage("Note: We've recognized a selected channel similar to our server-wide channel, '%s'. Please note that you have joined a different channel: '%s.'", name2, c_name_unedited);
        }
    }
