...
void WorldSession::HandleBfQueueRequest(WorldPacket& recvData)
{
    ...
    TC_LOG_ERROR("misc", "HandleBfJoinRequest: GUID:" UI64FMTD " ", (uint64)guid);
    if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldByGUID(guid))
...
{
    // toddo implement this with the guid received when the bit sequence will be found
    /*    ObjectGuid guid;

    guid[] = recvData.ReadBit();
    guid[] = recvData.ReadBit();
    guid[] = recvData.ReadBit();
    guid[] = recvData.ReadBit();
    guid[] = recvData.ReadBit();
    ...
    */
    ...
    // temp fix, removed this when the bit sequence is decrypt
    if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldWithPlayerInWar(_player))
        bf->AskToLeaveBattle(_player);
}
