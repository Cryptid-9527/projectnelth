...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (his_trade->IsAccepted())
    {
        ...
        if (auto log = CharacterDatabase.GetPreparedStatement(CHAR_INS_HISTORY_TRADE))
        {
            //log->setUInt64(0, 0);//guid, set by DB
            log->setUInt64(1, _player->GetGUID());//p1 GUID
            log->setUInt64(2, trader->GetGUID());//p2 GUID
            log->setUInt64(3, my_trade->GetMoney());//p1 money
            log->setUInt64(4, his_trade->GetMoney());//p2 money


            for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
            {
                if (auto item1 = myItems[i])//p1 items
                {
                    auto index{ 5 + (3 * i) };
                    log->setUInt32(uint8(index),   item1->GetEntry());
                    log->setUInt64(uint8(index + 1), item1->GetGUIDLow());
                    log->setUInt64(uint8(index + 2), item1->GetCount());
                }
                else
                {
                    auto index{ 5 + (3 * i) };
                    log->setUInt32(uint8(index), 0);
                    log->setUInt64(uint8(index + 1), 0);
                    log->setUInt64(uint8(index + 2), 0);
                }

                if (auto item2 = hisItems[i])//p2 items
                {
                    auto index{ 23 + (3 * i) };
                    log->setUInt32(uint8(index), item2->GetEntry());
                    log->setUInt64(uint8(index + 1), item2->GetGUIDLow());
                    log->setUInt64(uint8(index + 2), item2->GetCount());
                }
                else
                {
                    auto index{ 23 + (3 * i) };
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    //This player is shadow muted and should not be actually trading other players.
    if (auto p = _player)
