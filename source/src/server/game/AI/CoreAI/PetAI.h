...
class PetAI : public CreatureAI
{
    public:
        ...
        void AttemptBasicAttack();
        void AttemptAutoSpells();

        Unit* AutoCast_FindTarget(uint32 spell_id);
        bool AutoCast_ExtraChecks(Unit* target, uint32 spell_id);
        Unit* GetAutoCastTarget(bool friendly, float dist);
        static int Permissible(const Creature*);

        void KilledUnit(Unit* /*victim*/);
        void AttackStart(Unit* target);
        void AttackStart(Unit* target, uint32 spellId);
        void JustAttacked(Unit* victim);
