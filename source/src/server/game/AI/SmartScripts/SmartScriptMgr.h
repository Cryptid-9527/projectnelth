...
enum SMART_ACTION
{
    ...
    SMART_ACTION_MOVE_TO_POS                        = 69,     // PointId, xyz, ContactDistance
    ...
    SMART_ACTION_SET_ROOT                           = 121,
    SMART_ACTION_PLAY_ANIMKIT                       = 128,
    SMART_ACTION_SET_HEALTH_PCT                     = 142,    // percent
    SMART_ACTION_BECOME_PERSONAL_CLONE_FOR_PLAYER   = 149,    // summonType 1-8, duration in ms <--- This one could really save some time in a lot of scripts from cata zones
    //200+ is all non-TC custom actions
    SMART_ACTION_SET_OWNER                          = 201,
    SMART_ACTION_REMOVE_QUEST                       = 202,      //questID to remove from SMART_TARGET
    SMART_ACTION_MOVE_FALL                          = 203,
    SMART_ACTION_MOVE_TO_POS_FIXED_ORIENTATION      = 204,
    SMART_ACTION_ADD_UNIT_STATE                     = 205,
    SMART_ACTION_CLEAR_UNIT_STATE                   = 206,
    SMART_ACTION_CHANGE_VEHICLE_SEAT                = 207,
    SMART_ACTION_CAST_RANDOM_SPELL                  = 208,
    SMART_ACTION_CONNECT_TO_TRANSPORT               = 209,
    SMART_ACTION_SEND_TRANSPORT_INIT                = 210,
    SMART_ACTION_SEND_TRANSPORT_UPDATE              = 211,
    SMART_ACTION_SET_SPEED                          = 212,
    SMART_ACTION_CALL_CASTEDCREATUREORGO            = 227,     // CreatureId, SpellId

    SMART_ACTION_END                                = 228,
};
...
struct SmartAction
{
    ...
    union
    {
        ...
        struct
        {
            uint32 DoDisable;//1 = disable, 0 = enable
        } disableEvade;

        /*
         id, type (0 = oneShot, 1 = aiAnim, 2 = meleeAnim, 3 = movementAnim)
        */
        struct
        {
            uint32 id;
...
enum SMARTAI_TARGETS
{
    ...
    SMART_TARGET_CLOSEST_ENEMY                  = 26,
    SMART_TARGET_CLOSEST_FRIENDLY               = 27,
    SMART_TARGET_SUMMONS                        = 201,
    SMART_TARGET_END                            = 300
};
...
const uint32 SmartAIEventMask[SMART_EVENT_END][2] =
{
    ...
    {SMART_EVENT_COUNTER_SET,               SMART_SCRIPT_TYPE_MASK_CREATURE + SMART_SCRIPT_TYPE_MASK_GAMEOBJECT + SMART_SCRIPT_TYPE_MASK_AREATRIGGER + SMART_SCRIPT_TYPE_MASK_EVENT + SMART_SCRIPT_TYPE_MASK_GOSSIP + SMART_SCRIPT_TYPE_MASK_QUEST + SMART_SCRIPT_TYPE_MASK_SPELL + SMART_SCRIPT_TYPE_MASK_TRANSPORT + SMART_SCRIPT_TYPE_MASK_INSTANCE },
...
enum SmartCastFlags
{
    ...
    SMARTCAST_FORCE_DISMOUNT            = 0x80 // Dismount or Remove SHAPESHIFT
};
