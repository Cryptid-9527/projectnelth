...
void LFGQueue::RemoveFromCompatibles(uint64 guid)
{
    ...
    for (auto itNext = CompatibleMapStore.begin(); itNext != CompatibleMapStore.end();)
    {
...
{
    auto itr = CompatibleMapStore.find(key);
...
LfgCompatibility LFGQueue::CheckCompatibility(LfgGuidList check)
{
    ...
    for (auto it = check.begin(); it != check.end() && numLfgGroups <= 1 && numPlayers <= dungeon->GetMaxGroupSize(); ++it)
    {
        uint64 guid = (*it);
        LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
        if (itQueue == QueueDataStore.end())
        ...
        for (auto it2 = itQueue->second.roles.begin(); it2 != itQueue->second.roles.end(); ++it2)
    ...
    if (check.size() > 1)
    {
        for (auto it = check.begin(); it != check.end(); ++it)
        {
            ...
            for (auto itRoles = roles.begin(); itRoles != roles.end(); ++itRoles)
            {
        ...
        if (!LFGMgr::CheckGroupRoles(proposalRoles, dungeon))
        {
            ...
            {
                o.clear();
                for (auto it = debugRoles.begin(); it != debugRoles.end(); ++it)
    ...
    bool leader = false;
    for (auto itRoles = proposalRoles.begin(); itRoles != proposalRoles.end(); ++itRoles)
    {
        // Assing new leader
    ...
    for (auto itQueue = proposal.queues.begin(); itQueue != proposal.queues.end(); ++itQueue)
    {
        uint64 guid = (*itQueue);
        RemoveFromNewQueue(guid);
        RemoveFromCurrentQueue(guid);
...
void LFGQueue::UpdateQueueTimers(uint8 queueId, time_t currTime, uint32 &tankCount, uint32 &healerCount, uint32 &dpsCount)
{
    ...
    for (LfgQueueDataContainer::iterator itQueue = QueueDataStore.begin(); itQueue != QueueDataStore.end(); ++itQueue)
    {
        ...
        for (auto itPlayer = queueinfo.roles.begin(); itPlayer != queueinfo.roles.end(); ++itPlayer)
