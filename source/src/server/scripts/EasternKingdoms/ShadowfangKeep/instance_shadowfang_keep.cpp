...
public:
    ...
    struct instance_shadowfang_keep_InstanceMapScript: public InstanceScript
    {
        ...
        void OnCreatureCreate(Creature* creature)
        {
            ...
            switch (creature->GetEntry())
            {
                case NPC_QUESTGIVER_IVAR:
                case NPC_QUESTGIVER_BELMONT:
                    creature->setFaction(35);
                    break;
                case NPC_MAGISTRATE_HALEB: 
                case NPC_FORSAKEN_TROOPER:
                    Baron_Ash_prisonerGUID  = creature->GetGUID();
                    break;
        ...
        {
            //TC_LOG_ERROR("sql.sql", "player entered SFK with team %u", uint32(player->GetTeam()));
            InstanceScript::OnPlayerEnter(player);

            if (!TeamInInstance)
            {
                TeamInInstance = uint32(player->GetTeam());
                //TC_LOG_ERROR("sql.sql", "Setting SFK instance team to %u", uint32(TeamInInstance));
            }
        }
        ...
        uint64 GetData64(uint32 identifier) const
        {
            switch (identifier)
            {
                ...
                case DATA_ASHBURY_PRISONER:
                    return Baron_Ash_prisonerGUID;
                    break;
