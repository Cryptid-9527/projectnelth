...
public:
	...
	std::vector<ChatCommand> GetCommands() const override
	{
		...
		{
			{ "quest", SEC_CONSOLE, false, &HandleReportQuest, "" },
			{ "complete", SEC_CONSOLE, false, &HandleReportComplete, "" },
			{ "remove", SEC_CONSOLE, false, &HandleReportRemove, "" },
		};
		static std::vector<ChatCommand> commandTable =
		{
			{ "report", SEC_CONSOLE, false, NULL, "", reportCommandTable },
		};
		return commandTable;
	}
	...
	static bool HandleReportQuest(ChatHandler* handler, const char* args)
	{
		...
		{
			handler->SendSysMessage("You already completed this quest.");
			return true;
		}
		...
		{
		case 0:
			handler->SendSysMessage("This quest is not yet verified by the staff, please open a ticket");
			break;
		case 1:
			if (!ValidQuest(quest, player, handler, entry))
				return false;
			handler->SendSysMessage("This quest has been confirmed broken and should now be completed for you.");
			break;
		case 2:
	...
	static bool HandleReportComplete(ChatHandler* handler, const char* args)
	{
		...
		handler->SendSysMessage("Quest status changed to: BROKEN, and will be autocompleted on reports.");
		return true;
	}
	...
	static bool HandleReportRemove(ChatHandler* handler, const char* args)
	{
		...
		handler->SendSysMessage("Quest status changed to: WORKING.");
		return true;
	}
