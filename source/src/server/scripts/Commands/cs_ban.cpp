...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "account",        SEC_CONSOLE,  true,  &HandleUnBanAccountCommand,          "" },
            { "character",      SEC_CONSOLE,  true,  &HandleUnBanCharacterCommand,        "" },
            { "playeraccount",  SEC_CONSOLE,  true,  &HandleUnBanAccountByCharCommand,    "" },
            { "ip",             SEC_CONSOLE,  true,  &HandleUnBanIPCommand,               "" },
        };
        static std::vector<ChatCommand> banlistCommandTable =
        {
            { "account",        SEC_CONSOLE,  true,  &HandleBanListAccountCommand,        "" },
            { "character",      SEC_CONSOLE,  true,  &HandleBanListCharacterCommand,      "" },
            { "ip",             SEC_CONSOLE,  true,  &HandleBanListIPCommand,             "" },
        };
        static std::vector<ChatCommand> baninfoCommandTable =
        {
            { "account",        SEC_CONSOLE,  true,  &HandleBanInfoAccountCommand,        "" },
            { "character",      SEC_CONSOLE,  true,  &HandleBanInfoCharacterCommand,      "" },
            { "ip",             SEC_CONSOLE,  true,  &HandleBanInfoIPCommand,             "" },
        };
        static std::vector<ChatCommand> banCommandTable =
        {
            { "account",        SEC_CONSOLE,  true,  &HandleBanAccountCommand,            "" },
            { "character",      SEC_CONSOLE,  true,  &HandleBanCharacterCommand,          "" },
            { "playeraccount",  SEC_CONSOLE,  true,  &HandleBanAccountByCharCommand,      "" },
            { "ip",             SEC_CONSOLE,  true,  &HandleBanIPCommand,                 "" },
        };
        static std::vector<ChatCommand> commandTable =
        {
            { "ban",            SEC_CONSOLE,  true,  NULL,                                "", banCommandTable },
            { "baninfo",        SEC_CONSOLE,  true,  NULL,                                "", baninfoCommandTable },
            { "banlist",        SEC_CONSOLE,  true,  NULL,                                "", banlistCommandTable },
            { "unban",          SEC_CONSOLE,  true,  NULL,                                "", unbanCommandTable },
        };
        return commandTable;
    }
    ...
    static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
        ...
        switch (mode)
        {
            ...
            case BAN_IP:
            {
                //TC_LOG_ERROR("sql.sql", "Unban IP: %s", nameOrIP);
                ...
                if (!IsIPAddress(real_ip.c_str()))
                {
                    //TC_LOG_ERROR("sql.sql", "Unban IP false 1");
                    return false;
                }
                break;
            }
    ...
    static bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        pre_stmt->setString(0, std::string(ipStr));
        const char* real_ip{ "" };
        if (auto pre_result = LoginDatabase.Query(pre_stmt))
            real_ip = (*pre_result)[0].GetCString();

        /*
        PreparedStatement* pre_stmt2 = LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_MASKING_MASK_FROM_IP);
        pre_stmt->setString(0, std::string(ipStr));
        std::string real_ip{ "" };
        if (auto pre_result = LoginDatabase.Query(pre_stmt))
            real_ip = (*pre_result)[0].GetString();
        */
        ...
        handler->PSendSysMessage(LANG_BANINFO_IPENTRY,
            ipStr, fields[1].GetCString(), permanent ? handler->GetTrinityString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
    ...
    static bool HandleBanListIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (handler->GetSession())
        {
            ...
            {
                Field* fields = result->Fetch();
                auto matching_ip{ fields[0].GetCString() };

                //TC_LOG_ERROR("sql.sql", "checking db for mask %s", matching_ip);

                PreparedStatement* pre_stmt2 = LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_MASKING_MASK_FROM_IP);
                pre_stmt2->setString(0, std::string(matching_ip));
                if (auto pre_result2 = LoginDatabase.Query(pre_stmt2))
                {
                    Field* fields2 = pre_result2->Fetch();
                    auto mask{ fields2[0].GetUInt32() };
                    std::ostringstream ip_stream;
                    ip_stream << mask;

                    auto masked_ip{ ip_stream.str() };
                    auto ip_char{ masked_ip.c_str() };

                    //TC_LOG_ERROR("sql.sql", "results found: %s", masked_ip);
                    handler->PSendSysMessage("%s", ip_char);
                }
                else
                {
                    //TC_LOG_ERROR("sql.sql", "no results for LOGIN_SEL_IP_MASKING_MASK_FROM_IP %s", matching_ip);
                }
            }
            while (result->NextRow());
        }
