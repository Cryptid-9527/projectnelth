...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "listbinds",      SEC_CONSOLE,  false,  &HandleInstanceListBindsCommand,    "" },
            { "unbind",         SEC_CONSOLE,  false,  &HandleInstanceUnbindCommand,       "" },
            { "stats",          SEC_CONSOLE,  true,   &HandleInstanceStatsCommand,        "" },
            { "savedata",       SEC_CONSOLE,  false,  &HandleInstanceSaveDataCommand,     "" },
            { "setdata",        SEC_CONSOLE,  false,  &HandleInstanceSetDataCommand,      "" },
        };

        static std::vector<ChatCommand> commandTable =
        {
            { "instance",       SEC_CONSOLE,  true,   NULL,                               "", instanceCommandTable },
        };
    ...
    static bool HandleInstanceUnbindCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage("instance save binds unbound: %d", counter);
        counter = 0;
        ...
        handler->PSendSysMessage("player binds unbound: %d", counter);
        counter = 0;
        ...
        handler->PSendSysMessage("group instances unbound: %d", counter);
        counter = 0;

            if (auto times = player->GetInstanceTimes())
            if (times->size())
            if (auto trans = CharacterDatabase.BeginTransaction())
            {
                if (auto s = handler->GetSession())
                        for (auto this_time = times->begin(); this_time != times->end(); ++this_time)
                        {
                            counter++;
                            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ACCOUNT_INSTANCE_LOCK_TIMES);
                            stmt->setUInt32(0, s->GetAccountId());
                            trans->Append(stmt);
                        }
            ...
            handler->PSendSysMessage("save times unbound: %d", counter);
